\section{Розробка алгоритмів оптимального комплексування в інерціально-супутникових
систем навігації}
...
жорсткозв’язаной схемі використовуються лінеаризовані представлення залежностей  похибок 
...
розглядати поточні географічні координаті ( довготу $\lambda $, широту $\varphi $ і 
...
\item \eqref{eq:dVector} -- векторне рівняння відносно 
...
\item $R_{1} $ і $R_{2} $-- головні радіуси кривизни обраного земного еліпсоїда;
\end{ESKDexplanation}
...
\end{array}\] 
\begin{ESKDexplanation}
\item $a$,$e$-- велика піввісь і ексцентриситет земного еліпсоїда;
...
\Delta n_{h} ,} \end{array})^{T} $-- вектор проекцій суми переносного і коріолісового 
...
\item $g_{e}=\mu/a^{2}$, $\mu=398600,44\cdot 10^{9} \left[\text{м}^{3}/c^{2} \right]$
\end{ESKDexplanation}
...
на основі рівнянь \eqref{eq:coordinates}$\div $\eqref{eq:dVector} з використанням  
поточних показів ДКШ та акселерометрів можна отримати поточні значення  шуканих навігаційних 
параметрів ЛА.

При точному завдані вихідних умов і при точній  моделі прискорення сили ваги, а також 
при відсутності похибок інерціальних ДПІ і похибок обчислення в наслідок інтегрування 
рівнянь \eqref{eq:coordinates}$\div $\eqref{eq:dVector}  будуть отримані істинні 
значення основних навігаційних параметрів ЛА.

Похибки завдання вихідних координат і проекцій шляхової швидкості ЛА, похибки  початкового 
виставлення , аномальні варіації прискорення сили ваги, похибки інерціальних ДПІ, 
методичні похибки алгоритмів обчислення і похибки через  кінцеву довжину розрядній 
...
-- похибка БІНС у визначенні приведених координат місце знаходження ЛА; 
...
Аналіз показує, що еволюція параметрів $\alpha_{h}$, $\alpha_{E}$, $\alpha_{N}$ у часі описується наступною системою рівнянь:
\begin{equation} 
...
\item $\dot{\lambda }=\frac{V_{E} }{R\cos \varphi } ;$ $\dot{\varphi }=\frac{V_{N} }{R} $; $\varepsilon_{i}$
 (i=1,2,3)-- приведені похибки ДКШ БІНС;
\end{ESKDexplanation}
...
середніми значеннями і одиничними дисперсіями.
...
% $
% \scriptsize
% \left(\begin{array}{cccc cccc cccc cccc cccc cc}
...
% 
% {.} & 
% {\frac{1}{R}_{\text{З}} (u\cos \varphi +\frac{\dot{\lambda }}{\cos \varphi })} & {.} & 
% {\frac{tg\varphi }{R}} & {.} & {.} & 
% {\omega_{N}} & 
% {-\omega_{E}} & {.} & 
% {-b_{3,1}} & 
...
\item  $\sigma_{4} \div \sigma_{6} $ --  підсумковий ефект аномалій гравітаційного поля і похибок численного інтегрування 
...
ДПІ  $\bar{\varepsilon }_{A} $ і $\Delta \bar{0}_{A} $);
\item $\sigma_{a} $, $\sigma_{\omega } $ -- СКЗ білошумних складових погрішностей акселерометрів і ДКШ БІНС.
\end{ESKDexplanation}

Елементи матриць \textbf{F} і \textbf{G}, що випливає з аналізу співвідношень \eqref{eq:Fsdins_} 
...
% світла.
% 
% Похибки СНС при визначенні псевдодальностей $D_{sl}^{} $ і псевдошвидкостей $V_{sl}^{} $ можуть 
...
\begin{equation}
 \label{eq:ba}
\Delta h(t_{k} )=\Delta h_{\text{БВ}} +\sigma_{h} \eta_{n,k},               
\end{equation}

\begin{ESKDexplanation}
\item де $\Delta h_{\text{БВ}}$-- квазістаціонарна похибка виміру барометричної висоти, що обумовлена 
...
БІНС, одна -- систематична помилка БВ, 6 компонент -- корельовані 
...
До складу вектора спостережень  пропонується включити 8 компонент, у тому числі різницю 
оцінок висоти, видаваних  БІНС і БВ, 3 різниці координат і 3 різниці складові швидкості, вироблюваних БІНС і СНС відповідно, а також різниця оцінок висоти, видаваних БВ і СНС відповідно:
...
Другий підхід випливає з теоретичних моделей процесу оптимальної калманівської фільтрації 
...
\frown}{V}_{N} $, $\Delta \stackrel{\frown}{V}_{h} $ -- поточні оцінки помилок  
БІНС, одержувані на виході фільтра.
\end{ESKDexplanation}
Виправлення одержуваної в БІНС оцінки матриці орієнтації $B_{i} $ виконується 
...
Як відзначалося вище, для випадку грубих ДПІ роботу БІНС необхідно періодично коректувати. Період корекції ${T}_{\text{кор}} $ може вибиратися з умови:

\[\Delta \alpha ({T}_{\text{кор}})=\Delta \alpha_{\text{доп}} ,\] 
\begin{ESKDexplanation}
\item де $\Delta \alpha ({T}_{\text{кор}})$ -- оцінка максимальної помилки моделювання 
орієнтації осей географічної системи координат БІНС;
\item $\Delta \alpha_{\text{доп}} $ -- припустиме значення помилки, що забезпечує збереження 
лінійності моделі еволюції помилок БІНС.
\end{ESKDexplanation}
...
\item $\Delta \alpha_{0} $ -- максимальне значення помилки початкової виставки БІНС ;
\item $\Delta a$,$\varepsilon $ -- максимальне значення помилки інерціальних ДПІ БІНС.
\end{ESKDexplanation}
...
\eqref{eq:correct_B};
 \item оновлюються оцінки приведених помилок датчиків БІНС  за формулами:
\[\begin{array}{l} 
...
помилок у точці корекції БІНС;
 \item обнуляються компоненти вектора стану $X_{p}$ 1$\div$ 15, що відповідають помилкам 
БІНС.
\end{itemize}
...
% 
% $F_{P,K} ,G_{P,K} $- матриці розміру $N_{S} \times N_{S} (N_{S} =16+2*(N+1))$ і $N_{S} 
